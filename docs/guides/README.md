...
- [Embedding the Core](./embedding-the-core.md) — manually assemble, initialize, drive, reset, and recompose a CHIP-8 machine using the public Core API, including Classic CHIP-8, CHIP-48, and SUPER-CHIP profiles, display specifications, font selection, `ExitState`, and persistent `RplFlags`.
...
- [Web application](./web-application.md) — understand the browser host, including selectable Classic/CHIP-48/SUPER-CHIP profiles, profile recomposition, shared SUPER-CHIP framebuffer presentation, persistent RPL ownership, the responsive play-and-inspection workspace, physical and virtual input, Web Audio, execution controls, passive CPU/disassembly/trace inspection, appearance themes, and ROM lifecycle.
...
- [Release checklist](./release-checklist.md) — prepare, verify, manually exercise affected host behavior, and tag a Chip8NX milestone release using the repository's existing validation contract.
