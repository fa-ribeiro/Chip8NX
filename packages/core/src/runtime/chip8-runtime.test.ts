...
function createRuntime(
  ...
  scheduledCpuExecutionGate: () => boolean = () => true,
): RuntimeHarness {
...
Deno.test(
  "Chip8Runtime pauses before scheduled CPU execution when the execution gate denies",
  () => {
    let gateCalls = 0;

    const { runtime, clock, context } = createRuntime(Frequency.fromInteger(500n), () => {
      gateCalls += 1;
    ...
    assertEquals(context.registers.get(registerIndex(0)), byte(0));
    assertEquals(context.programCounter.getValue(), CLASSIC_CHIP8_PROFILE.programStartAddress);
    assertEquals(runtime.isPaused, true);
  },
);

Deno.test("Chip8Runtime stops scheduled CPU catch-up when the execution gate denies", () => {
  let gateCalls = 0;
  ...
  /*
   * 500 Hz over 20 ms would normally produce ten CPU executions.
   * The third scheduled attempt is denied, so only the first two execute.
   */
  advanceClock(clock, 20_000_000n);
  runtime.tick();

  assertEquals(gateCalls, 3);
  assertEquals(context.registers.get(registerIndex(0)), byte(2));
  assertEquals(
    context.programCounter.getValue(),
    address(CLASSIC_CHIP8_PROFILE.programStartAddress + 4),
  );
  assertEquals(runtime.isPaused, true);
});

Deno.test("Chip8Runtime step bypasses the scheduled CPU execution gate", () => {
  let gateCalls = 0;

  const { runtime, context } = createRuntime(Frequency.fromInteger(500n), () => {
    gateCalls += 1;
...
Deno.test(
  "Chip8Runtime processes timer work before a denied CPU attempt on an exact deadline tie",
  () => {
    let gateCalls = 0;
    ...
    assertEquals(delayTimer.getValue(), byte(0));
    assertEquals(gateCalls, 1);
    assertEquals(context.registers.get(registerIndex(0)), byte(0));
    assertEquals(context.programCounter.getValue(), CLASSIC_CHIP8_PROFILE.programStartAddress);
    assertEquals(runtime.isPaused, true);
