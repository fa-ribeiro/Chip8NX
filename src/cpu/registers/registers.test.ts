...
Deno.test("Registers registers are independent", () => {
  ...
  assertEquals(registers.get(registerIndex(0)), byte(0x11));

  assertEquals(registers.get(registerIndex(1)), byte(0x22));
});
...
Deno.test("Registers VF can be addressed like any other register", () => {
  ...
  assertEquals(registers.get(registerIndex(0xf)), byte(0xff));
});
...
Deno.test("Registers snapshot is independent from the register bank", () => {
  ...
  registers.set(registerIndex(0), byte(0x34));
